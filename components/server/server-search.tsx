...
export const ServerSearch = ({
    ...
    const onClick = ({ id, type }: { id: string, type: 'channel' | 'member' }) => {
        setOpen(false)
        if (type === 'channel') {
            return router.push(`${params?.serverId}/channels/${id}`)
        } else if (type === 'member') {
            return router.push(`/profile/${params?.serverId}/conversations/${id}`)
        }
    ...
    return (
        <>
         ...
         gap-x-2 w-full hover:bg-zinc-100/10 dark:hover:bg-zinc-700 transition">
